...
def fetch_daily_candles(pair: str, n_candles: int, log=print) -> dict | None:
    """Fetch daily candles from Yahoo Finance for pair, in the same Twelve
    Data time_series dict shape as fetch_4h_candles()/fetch_1h_candles() so
    all three can be fed through technical._frame_from_td() uniformly.

    2026-09-06: added for the intraday 4H book (src/intraday_4h.py) -- its
    higher-timeframe (daily) filter needs real SMA200/ribbon-length history,
    which the 60-day-of-1H window fetch_4h_candles() uses is far too short
    for, so this pulls daily bars directly (2y is comfortably enough and
    fast) rather than resampling from that same short window.
    """
    try:
        import yfinance as yf
    except ImportError:
        log("[YF-DAILY] yfinance not installed")
        return None
    ...
    try:
        ...
        log(f"[YF-DAILY] fetch failed for {symbol} ({pair}): {exc}")
        return None

    if df is None or df.empty:
        log(f"[YF-DAILY] No daily data returned for {symbol} ({pair})")
        return None

    df.columns = [str(c).lower() for c in df.columns]
    for col in ("open", "high", "low", "close"):
        if col not in df.columns:
            log(f"[YF-DAILY] Missing column '{col}' for {symbol}")
            return None
    df = df[["open", "high", "low", "close"]].dropna()
    if df.empty:
        log(f"[YF-DAILY] All daily candles NaN for {symbol}")
        return None
    ...
    if df.empty:
        log(f"[YF-DAILY] Not enough daily data for {symbol}")
        return None
    ...
    return {
        "meta":   {"symbol": pair, "source": "Yahoo Finance (Daily)"},
        "values": values,
        "status": "ok",
    }
