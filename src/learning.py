...
def compute_stats() -> dict:
    ...
    return {
        ...
        "closed": len(closed),
        "wins": sum(1 for r in closed if r.get("status") in _DECISIVE_STATUSES and _is_win(r)),
        "losses": sum(1 for r in closed if r.get("status") in _DECISIVE_STATUSES and not _is_win(r)),
        "win_rate": wr,
        "decisive": n,
