...
| `vbook_G_mechanical_reversion` | **Not promotable — registered 2026-09-2X, 0 live regimes recorded yet** | Live pilot of `rib_against AND osc_agrees` (see `PROPOSAL_mechanical_reversion_engine.md` and `src/mechanical_reversion.py`), no confidence floor, no dd_mode gate, no LLM call, both directions (the BUY-only refinement's currency attribution was unstable between discovery and holdout — left for real forward data to resolve, not baked in). **n_fire/n_no_fire count REGIMES, not raw trades** (`BookConfig.regime_aware_promotion=True`, `_regime_dedup_allows_recording()` in `src/virtual_books.py`) — a real cluster-bootstrap re-analysis of the backtest found the naive per-row test on `rib_against` alone (raw p=0.00015) was NOT actually significant once regime-clustering was honestly accounted for (cluster-bootstrap p=0.052, regimes ran up to 232 consecutive calendar days in the data), while the combined signal this book trades did survive (cluster-bootstrap p=0.0060, effect +6.2pp WR, 95% CI [+1.8, +11.0]pp). Bar: n≥30/30 regimes (not raw trades — a materially harder real bar than the same number would be for a per-row-counted rule), p<0.05 (Bonferroni-corrected by active-rule count as usual). |
