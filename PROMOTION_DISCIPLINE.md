...
| `technical_carries_divergence` | **Not promotable — registered on the discovery sample, 0 fresh evaluations yet** | divergence = tech_score − mean(fund/sent/pos/macro) ≥ 3.0 (the CHF/JPY #7325 postmortem shape). Discovery sample (research_trades.csv, n=41): WR=17.1% vs 26.2% (divergence≤1, n=909), PF=0.243 vs 0.754 — real, large gap, but p=0.096 (not <0.05) and non-monotonic at the tail (n=18 @divergence≥3.5 bounced to WR=33.3%). Bar: n≥100/100, p<0.05, on fresh out-of-sample data — nothing currently feeds `record_evaluation()` for this rule, so it will not accumulate evidence on its own; wiring that in is a separate follow-up, not yet done. |
